...
class k_means_gs : public k_means_interface
{
	...
	virtual bool UpdateClustersID()
	{
		...
		for (numclustype the_C = 0; the_C < the_k; the_C++)
		{
			...
			for (size_t pi = 0; pi < cluster_mem[the_C].size(); pi++)
			{
				...
				for (numclustype cj = 0; cj < the_k; cj++)
				{
					distype cur_dis = dis_metric->measure((*inst)[point], centers[cj].data());
...
public:
	...
	virtual string name()const { if (!seedInitializer)return "km-gs"; return "km-gs-" + seedInitializer->name(); }
};
