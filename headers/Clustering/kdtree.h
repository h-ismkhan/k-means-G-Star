...
#include <array>
class MyPoint : public std::array<coordtype, 3>
{
public:
	...
	// the constructors
	MyPoint() {}
	MyPoint(coordtype x, coordtype y, coordtype z)
	{
		(*this)[0] = x;
		(*this)[1] = y;
		(*this)[2] = z;
...
class MyPoint_5 : public std::array<coordtype, 5>
{
public:
	...
	// the constructors
	MyPoint_5() {}
	MyPoint_5(coordtype v, coordtype w, coordtype x, coordtype y, coordtype z)
	{
		(*this)[0] = v;
		(*this)[1] = w;
		(*this)[2] = x;
	...
	MyPoint_5(const vector<coordtype> &point_vec)
	{
...
namespace kdt
{
	...
	*/
	...
	public:
		...
		*/
		std::vector<size_t> radiusSearch(const PointT& query, double radius) const
		{
	...
	private:
		...
		*/
		...
		/** @brief Searches neighbors within radius.
		*/
		void radiusSearchRecursive(const PointT& query, const Node* node, std::vector<size_t>& indices, double radius) const
		{
			if (node == nullptr)
				return;
